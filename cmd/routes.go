...
func (app *application) routes() http.Handler {
    ...
    mux.HandleFunc("/link-applemusic", session.WithAuth(handleAppleMusicLink(app.pages), app.sessionManager))
    ...
    // Apple Music developer token (protected with session auth)
    mux.HandleFunc("/api/v1/applemusic/developer-token", session.WithAuth(app.appleMusicService.HandleDeveloperToken, app.sessionManager))
